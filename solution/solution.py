...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            "Score each criterion from 0.0 to 1.0 and reply with a JSON object "
            "mapping each criterion name to its score."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            criterion_scores = item.get("scores", {})
            if criterion_scores:
                item_averages.append(sum(criterion_scores.values()) / len(criterion_scores))

        overall_avg = sum(item_averages) / len(item_averages) if item_averages else 0.0

        positional_bias = False
        if len(item_averages) >= 2:
            first_position = item_averages[0::2]
            second_position = item_averages[1::2]
            if first_position and second_position:
                avg_first = sum(first_position) / len(first_position)
                avg_second = sum(second_position) / len(second_position)
                positional_bias = avg_first > avg_second + 0.1
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def _averages(results: list) -> dict[str, float]:
            n = len(results)
            if n == 0:
        ...
        new_avg = _averages(new_results)
        baseline_avg = _averages(baseline_results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggestion = suggestions[i] if i < len(suggestions) else "Review manually"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter unsupported claims and "
                "strengthen faithfulness guardrails"
            )
        ...
        if categories.get("incomplete"):
            suggestions.append(
                "Increase chunk size or retrieval top-k in the RAG pipeline to reduce "
                "context fragmentation"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Add a routing/classification step to detect and redirect off-topic queries"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Loosen overly strict guardrails that are causing unnecessary refusals"
            )

        generic_suggestions = [
            "Add few-shot examples showing complete, well-grounded answers",
            "Expand golden dataset coverage for the affected failure categories",
            "Review retrieval configuration (chunk size, top-k, embedding model)",
        ]
